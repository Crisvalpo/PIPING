...
export async function getProjectStats(projectId: string) {
    try {
        ...
        const vigentesCount = vigentesExact || 0
        const pendientesCount = pendientes || 0
        const eliminadosCount = Math.max(0, totalCount - vigentesCount)

        return {
            total: totalCount,
            vigentes: vigentesCount,
            eliminados: eliminadosCount,
            pendientesSpooling: pendientesCount
        }
    } catch (error) {
