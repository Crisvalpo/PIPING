...
export default function EngineeringManager({ projectId }: EngineeringManagerProps) {
    ...
    async function handleDeleteRevision(revisionId: string) {
        if (!confirm('¿Estás seguro de eliminar esta revisión? Desaparecerá de la lista.')) return
        try {
    ...
    const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        try {
            ...
            if (uploadMode === 'ANNOUNCEMENT') {
                ...
                if (missingSheets.length > 0) throw new Error(`Faltan hojas requeridas: ${missingSheets.join(', ')}`)
    ...
    const handleDownloadTemplate = () => {
        ...
        if (uploadMode === 'ANNOUNCEMENT') {
            const data = [{ "N°ISOMÉTRICO": "Ejemplo", "REV. ISO": 0 }]
            XLSX.utils.book_append_sheet(wb, XLSX.utils.json_to_sheet(data), "Anuncios")
            XLSX.writeFile(wb, "Template_Anuncio.xlsx")
        } else {
            XLSX.utils.book_append_sheet(wb, XLSX.utils.json_to_sheet([{}]), "bolted_joints")
            XLSX.utils.book_append_sheet(wb, XLSX.utils.json_to_sheet([{}]), "spools_welds")
            XLSX.utils.book_append_sheet(wb, XLSX.utils.json_to_sheet([{}]), "material_take_off")
                                            ...
                                            <td className="p-3 text-right">
                                                <div className="flex items-center justify-end gap-2">
                                                    ...
                                                    <button
                                                        ...
                                                        onClick={() => activeRev && handleDeleteRevision(activeRev.id)}
                            ...
                            <table className="w-full text-left border-collapse">
                                <thead>
                                    <tr className="text-gray-500 border-b border-white/10 text-xs uppercase tracking-wider">
                                        ...
                                        <th className="p-3">Spooling</th>
                                        <th className="p-3 text-right">Archivo</th>
                                    </tr>
                                </thead>
                                <tbody className="text-sm">
                                    {viewingHistory.revisions?.map((rev: any) => {
                                        ...
                                        return (
                                            <tr key={rev.id} className="border-b border-white/5 hover:bg-white/5 transition-colors">
                                                ...
                                                <td className="p-3 text-right">
                                                    <button
                                                        className={`p-2 hover:bg-white/10 rounded transition-colors ${!hasFiles ? 'opacity-30 cursor-not-allowed' : 'text-blue-400'}`}
